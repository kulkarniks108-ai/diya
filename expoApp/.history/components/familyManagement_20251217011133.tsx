...
export default function FamilyManagementCard() {
  ...
  useEffect(() => {
    // Invoke once on mount to start listening; avoid depending on
    // store function identity to prevent repeated subscriptions.
    useAuthStore.getState().listenToFamilyMembers();
    return () => {
      // Store owns listener lifecycle; no local cleanup needed.
    };
  }, []);
